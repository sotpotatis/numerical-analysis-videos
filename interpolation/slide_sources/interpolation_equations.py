...
def generate_interpolation_polynomial_equation(
    ...
    for i in range(len(coefficient_numbers)):
        ...
        if i < len(coefficient_numbers) - 1:
            equation += "\cdot {%r}^{%d}" % (
                round(point_x, round_points_to_decimals),
...
def illustrate_interpolation_equations(
    ...
    arrow_colors = cycle_through_color_wheel(number_of_colors=len(points_to_include))
    for i in range(len(points_to_include)):
        ...
        scene_reference.add(arrow_to_point, point_equation_system)
...
def create_interpolation_matrix(
    ...
    return matrix_system_latex, [a_matrix, b_matrix, x_matrix]
